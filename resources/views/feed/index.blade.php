<x-app-layout>
            ...
            <div class="space-y-6">
                @forelse($posts as $post)
                    <div class="bg-white rounded-lg shadow-md overflow-hidden">
                                ...
                                @foreach($post->media as $media)
                                    @if($media->media_type === 'image')
                                        <img src="{{ asset('storage/' . $media->file_path) }}" 
                                             alt="Post image" 
                                             class="w-full {{ $post->media->count() === 1 ? 'max-h-[600px]' : 'h-64' }} object-cover cursor-pointer"
                                             onclick="openImageModal('{{ asset('storage/' . $media->file_path) }}')">
                                    @else
                                        <video controls class="w-full {{ $post->media->count() === 1 ? 'max-h-[600px]' : 'h-64' }} object-cover">
                        ...
                        <div class="border-t border-gray-200 px-4 py-3">
                            ...
                            <div class="flex items-center justify-around border-t border-gray-200 pt-3">
                                <!-- Like Button -->
                                <form method="POST" action="{{ route('posts.react', $post) }}" class="inline-block">
                                    ...
                                    <button type="submit" class="flex items-center space-x-2 text-gray-600 hover:text-blue-600 transition">
                                        <svg class="w-5 h-5 {{ $post->reactions->where('user_id', auth()->id())->isNotEmpty() ? 'fill-blue-600 text-blue-600' : '' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <!-- Comment Button -->
                                <button onclick="document.getElementById('comment-form-{{ $post->id }}').classList.toggle('hidden')" class="flex items-center space-x-2 text-gray-600 hover:text-green-600 transition">
                        ...
                        <!-- Comment Form -->
                        <div id="comment-form-{{ $post->id }}" class="hidden border-t border-gray-200 px-4 py-3">
                                ...
                                <div class="flex space-x-3">
                                    ...
                                    <div class="flex-1">
                                        <textarea name="content" 
                                                  ...
                                                  required></textarea>
                                        <button type="submit" class="mt-2 bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition text-sm">
                                            Post Comment
                                        </button>
                                    </div>
                                </div>
                        ...
                        @if($post->comments->isNotEmpty())
                            <div class="border-t border-gray-200 px-4 py-3 space-y-3">
                                    ...
                                    <div class="flex space-x-3">
                                        ...
                                        <div class="flex-1">
                                            <div class="bg-gray-100 rounded-lg px-3 py-2">
                                                <a href="{{ route('profile.show', $comment->user->username) }}" class="font-semibold text-sm text-gray-900 hover:underline">
                                                    {{ $comment->user->name }}
                                                </a>
                                ...
                                @if($post->comments->count() > 3)
                                    <a href="{{ route('posts.show', $post) }}" class="text-sm text-blue-600 hover:underline">
                ...
                @empty
                    <div class="bg-white rounded-lg shadow-md p-8 text-center">
                        ...
                        <p class="text-gray-600 mb-4">Start following people to see their posts in your feed!</p>
                        <a href="{{ route('posts.create') }}" class="inline-block bg-blue-600 text-white px-6 py-2 rounded-lg hover:bg-blue-700 transition">
    ...
    <div id="imageModal" class="hidden fixed inset-0 bg-black bg-opacity-90 z-50 flex items-center justify-center p-4" onclick="closeImageModal()">
        <img id="modalImage" src="" alt="Full size image" class="max-h-full max-w-full">
    </div>
